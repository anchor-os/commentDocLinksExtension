...
test("missing documentation file is reported", () => {
  ...
  const broken = collectBrokenReferences(document, makeFileSystem({}), "");
...
test("existing file with matching anchor is not reported", () => {
  const document = makeDocument(["// see documentation/a.md#real-anchor"]);

  const broken = collectBrokenReferences(
    document,
    makeFileSystem({
      "documentation/a.md": "## src/util/foo.js — real-anchor\n",
    }),
    "",
  );
...
test("bare-CR target with matching anchor is not reported", () => {
  ...
  const broken = collectBrokenReferences(
    document,
    makeFileSystem({
      "documentation/a.md": "intro\r## src/util/foo.js — real-anchor\r",
    }),
    "",
  );
...
test("line reference within the file is not reported", () => {
  const document = makeDocument(["// see documentation/a.md#L2"]);
...
test("unreadable target file is skipped, not reported", () => {
  const document = makeDocument(["// see documentation/a.md#anything"]);
...
test("markdown heading to missing source file is reported", () => {
  ...
  const broken = collectBrokenReferences(document, makeFileSystem({}), "documentation/a.md");
...
test("markdown heading with missing source anchor is reported", () => {
  ...
  const broken = collectBrokenReferences(
    document,
    makeFileSystem({
      "src/util/foo.js": "// see documentation/a.md#different-anchor\n",
    }),
    "documentation/a.md",
  );
...
test("markdown heading with matching source anchor is not reported", () => {
  const document = makeDocument(["## src/util/foo.js — real-anchor"], "markdown");

  const broken = collectBrokenReferences(
    document,
    makeFileSystem({
      "src/util/foo.js": "// see documentation/a.md#real-anchor\n",
    }),
    "documentation/a.md",
  );
...
test("markdown heading pointing at a markdown target skips anchor check", () => {
  const document = makeDocument(["## documentation/other.md — anything"], "markdown");
...
test("references inside block comments are checked", () => {
  const document = makeDocument(["/*", " * see documentation/missing.md", " */"]);

  const broken = collectBrokenReferences(document, makeFileSystem({}), "");

  assert.equal(broken.length, 1);
  assert.equal(broken[0].line, 1);
  assert.equal(broken[0].message, "Documentation file not found: documentation/missing.md");
});

test("issue and API references are external, not broken", () => {
  const document = makeDocument([
    "// track #123 and API:Checkout",
