...
test("targetsReferencedBy extracts resolved comment targets", () => {
    ...
    const targets = targetsReferencedBy(
        document,
        contextFor("/root")
    );
    ...
    assert.deepEqual(
        [...targets],
        [
            "/root/documentation/a.md",
            "/root/documentation/b.md"
        ]
    );
});
...
test("targetsReferencedBy extracts resolved markdown targets", () => {
    ...
    assert.deepEqual(
        [...targets],
        [
            "/root/src/util/foo.js",
            "/root/src/util/bar.js",
            "/root/src/util/baz.js"
        ]
    );
});
...
test("targetsReferencedBy records paths even when the file does not exist", () => {
    const document = makeDocument([
        "// see documentation/not-created-yet.md"
    ]);
    ...
    assert.deepEqual(
        [...targets],
        ["/root/documentation/not-created-yet.md"]
    );
});
...
test("references that fail to resolve produce no dependency", () => {
    ...
    const context = {
        resolveTargetPath(relativePath) {
            return relativePath === "blocked.md"
                ? null
                : `/root/${relativePath}`;
        }
    };
...
test("dependentsOf returns every source that references a target", () => {
    ...
    assert.deepEqual(
        index.dependentsOf("/ws/documentation/api.md"),
        ["/ws/src/foo.js", "/ws/main.tf"]
    );

    assert.deepEqual(
        index.dependentsOf("/ws/documentation/other.md"),
        ["/ws/main.tf"]
    );

    assert.deepEqual(
        index.dependentsOf("/ws/documentation/unrelated.md"),
        []
    );
});
...
test("replacing a source's references removes stale reverse entries", () => {
    ...
    index.set(
        "/ws/src/foo.js",
        new Set(["/ws/documentation/a.md", "/ws/documentation/b.md"])
    );
    ...
    assert.deepEqual(
        index.dependentsOf("/ws/documentation/c.md"),
        ["/ws/src/foo.js"]
    );
...
test("worktrees with the same relative path stay isolated", () => {
    ...
    assert.deepEqual(
        index.dependentsOf(
            "/repo/worktrees/feature/documentation/api.md"
        ),
        ["/repo/worktrees/feature/src/foo.js"]
    );

    assert.deepEqual(
        index.dependentsOf("/repo/documentation/api.md"),
        ["/repo/src/foo.js"]
    );
});
...
test("documentsToRefresh returns only the changed document and its dependents", () => {
    ...
    index.set(
        "/ws/src/foo.js",
        new Set(["/ws/documentation/api.md"])
    );
    index.set(
        "/ws/src/bar.js",
        new Set(["/ws/documentation/other.md"])
    );
    index.set(
        "/ws/main.tf",
        new Set(["/ws/documentation/api.md"])
...
test("changing a target refreshes dependents and never unrelated documents", () => {
    ...
    assert.deepEqual(
        ...
        ["/ws/documentation/other.md", "/ws/src/bar.js"]
    );
});
...
test("a changed version must be re-scanned and replaces the last targets", () => {
    ...
    assert.deepEqual(index.targetsOf("/ws/src/foo.js"), [
        "/ws/documentation/a.md"
    ]);

    index.set(
        "/ws/src/foo.js",
        new Set(["/ws/documentation/b.md"]),
        "v2"
    );
