...
test("ignores references outside comments", () => {
  const scanned = scanDocumentForReferences(
    makeDocument(["const url = 'documentation/a.md';", "// see documentation/b.md"]),
  );
...
test("scans inside multiline block comments", () => {
  const scanned = scanDocumentForReferences(
    makeDocument(["/*", " * see documentation/missing.md", " */"]),
  );
...
test("unsupported languages return no references", () => {
  const scanned = scanDocumentForReferences(makeDocument(["# see documentation/a.md"], "css"));

  assert.deepEqual(scanned, []);
});

test("marks whole-line hash comment languages", () => {
  const scanned = scanDocumentForReferences(makeDocument(["# see documentation/a.md"], "python"));
...
test("php attribute lines are not scanned as references", () => {
  const scanned = scanDocumentForReferences(
    makeDocument(
      ["<?php", '#[Route("/api/checkout", name: "checkout")]', "# see documentation/a.md"],
      "php",
    ),
  );
...
test("php HTML apostrophes do not hide later comments", () => {
  const scanned = scanDocumentForReferences(
    makeDocument(
      ["<p>It's here</p>", "<?php", "// see documentation/a.md", "?>", "<p>More text</p>"],
      "php",
    ),
  );
