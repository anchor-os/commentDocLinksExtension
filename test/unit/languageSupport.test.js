...
test("yaml quoted strings are not comments", () => {
    for (const line of [
        ...
        assert.deepEqual(
            ranges,
            [],
            `${line} should produce no comment`
        );
    }
});

test("yaml # needs a preceding whitespace to start a comment", () => {
    const plainScalar = getCommentRanges(
        "yaml",
        "key: value#123",
        { inBlockComment: false, inString: null }
    );
    ...
    const comment = getCommentRanges(
        "yaml",
        "key: value # see documentation/file.md",
        { inBlockComment: false, inString: null }
    );
...
test("yaml block scalar content hides # across lines", () => {
    ...
    const comment = getCommentRanges(
        "yaml",
        "# real comment documentation/file.md",
        state
    );
...
test("yaml indented block scalar resumes at the same indent", () => {
    ...
    const code = getCommentRanges(
        "yaml",
        "  # back to code documentation/file.md",
        state
    );
...
test("terraform heredoc body hides # and // across lines", () => {
    ...
    const comment = getCommentRanges(
        "terraform",
        "# now a comment documentation/file.md",
        state
    );
...
test("velocity ## comments start anywhere outside strings", () => {
    const after = getCommentRanges(
        "velocity",
        "#set($x = 1) ## see documentation/file.md",
        { inBlockComment: false, inString: null }
    );
    ...
    const directive = getCommentRanges(
        "velocity",
        "#if($x) ## see documentation/file.md #end",
        { inBlockComment: false, inString: null }
    );
...
test("velocity strings hide ## and #*", () => {
    const double = getCommentRanges(
        "velocity",
        '#set($msg = "see documentation/file.md ##123")',
        { inBlockComment: false, inString: null }
    );

    assert.deepEqual(double, []);

    const single = getCommentRanges(
        "velocity",
        "#set($msg = '## not a comment')",
        { inBlockComment: false, inString: null }
    );
