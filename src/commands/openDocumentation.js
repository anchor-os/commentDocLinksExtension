...
import { openReference } from "./openReference.js";
 ...
 */
export async function openDocumentationFile(relativePath, anchor, line, sourceDocumentPath) {
...
export function registerOpenDocumentationCommand(context) {
  context.subscriptions.push(
    vscode.commands.registerCommand(
       ...
       * @param {string} [sourceDocumentPath]
       */
      async (relativePath, anchor, line, sourceDocumentPath) => {
        if (typeof relativePath !== "string") {
          return;
        }

        try {
          await openDocumentationFile(relativePath, anchor, line, sourceDocumentPath);
        } catch (error) {
          console.error(error);

          vscode.window.showErrorMessage(`Unable to open documentation: ${relativePath}`);
        }
      },
    ),
  );
