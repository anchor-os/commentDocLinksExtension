...
import { hasExactSourceReference } from "../services/sourceResolver.js";
 ...
 */
export function collectBrokenReferences(document, context, relativeDocumentationPath) {
  const referenceContext = normalizeContext(context);

  if (document.languageId === "markdown") {
    return collectBrokenMarkdownReferences(document, referenceContext, relativeDocumentationPath);
  }

  return collectBrokenCommentReferences(document, referenceContext);
}
 ...
 * @param {string} relativeDocumentationPath
 */
function collectBrokenMarkdownReferences(document, context, relativeDocumentationPath) {
  const broken = [];
  ...
  for (let i = 0; i < document.lineCount; i++) {
    ...
    const targetPath = context.resolveTargetPath(parsed.source);

    if (targetPath === null || !context.fs.exists(targetPath)) {
      broken.push({
        line: i,
        start: parsed.start,
        end: parsed.end,
    ...
    const languageId = getLanguageIdFromExtension(parsed.source);

    if (!parsed.anchor || languageId === null || languageId === "markdown") {
      continue;
    }

    const sourceText = context.fs.readText(targetPath);

    if (sourceText === null) {
      continue;
    }

    const sourceDoc = documentFromText(sourceText, languageId);

    if (!hasExactSourceReference(sourceDoc, relativeDocumentationPath, parsed.anchor)) {
      broken.push({
        line: i,
        start: parsed.start,
        end: parsed.end,
        message: `Source anchor not found: ${parsed.anchor}`,
      });
    }
  }
