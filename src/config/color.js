...
const COLOR_COMPONENT_PATTERN = /^[+-]?(?:\d+(?:\.\d+)?|\.\d+)(?:deg|rad|grad|turn|%)?$/i;

const ALPHA_COMPONENT_PATTERN = /^[+-]?(?:\d+(?:\.\d+)?|\.\d+)(?:%)?$/i;
...
function isFunctionalColor(value) {
  ...
  const color = slashIndex === -1 ? body : body.slice(0, slashIndex);
  const alpha = slashIndex === -1 ? null : body.slice(slashIndex + 1);

  if (alpha !== null && (alpha.includes("/") || color.length === 0)) {
    return false;
  }

  const legacySyntax = color.includes(",");

  const components = (legacySyntax ? color.split(",") : color.trim().split(/\s+/)).map((part) =>
    part.trim(),
  );
  ...
  if (components.length !== 3 || components.some((part) => !COLOR_COMPONENT_PATTERN.test(part))) {
    return false;
  }
  ...
  if (alpha !== null) {
    ...
    if (alphaComponent.length === 0 || !ALPHA_COMPONENT_PATTERN.test(alphaComponent)) {
      return false;
    }
  }
