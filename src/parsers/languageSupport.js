...
function getPhpCommentRanges(line, state) {
  ...
  while (i < line.length) {
    ...
    if (char === "<" && next === "<" && line[i + 2] === "<") {
      const label = line.slice(i + 3).match(/^[ \t]*(['"]?)([A-Za-z_][A-Za-z0-9_]*)\1/);
...
function yamlBlockScalarHeaderIndent(line) {
  ...
  for (let i = 0; i < line.length; i++) {
    ...
    const opensScalar = char === ":" && yamlBlockScalarValue(line, i + 1) !== -1;
...
function yamlBlockScalarValue(line, index) {
  ...
  while (line[k] === "+" || line[k] === "-" || (line[k] >= "1" && line[k] <= "9")) {
    k++;
  }
...
function getTerraformCommentRanges(line, state) {
  ...
  while (i < line.length) {
    ...
    if (quote) {
      if (quote.startsWith("heredoc:")) {
        ...
        if (
          ...
          (trimmed.length > terminator.length && !/^\s/.test(trimmed.slice(terminator.length)))
        ) {
          return ranges;
        }
    ...
    if (char === "<" && next === "<" && (i === 0 || /[\s=([,]/.test(line[i - 1]))) {
      let j = i + 2;
      ...
      const label = line.slice(j).match(/^[ \t]*(['"]?)([A-Za-z_][A-Za-z0-9_]*)\1/);
