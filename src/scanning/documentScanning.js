...
export function createDocumentScanner(deps) {
    ...
    const queueDocumentAtPath = (fsPath, priority) => {
        scanner.enqueue({
            ...
            run: async () => {
                ...
                if (
                    version === null ||
                    dependencyIndex.isCurrent(fsPath, version)
                ) {
                    return;
                }
                ...
                indexDocument(
                    ...
                    documentFromText(
                        ...
                        getLanguageIdFromExtension(fsPath) ??
                            "markdown"
                    ),
                    version
                );
    ...
    const queueReferencedTargetsOfPath = (sourcePath) => {
        for (const targetPath of
            dependencyIndex.targetsOf(sourcePath)) {
    ...
    const queueOpenDocument = (document, priority) => {
        ...
        if (
            dependencyIndex.isCurrent(fsPath, document.version)
        ) {
            return;
        }
    ...
    const queueAllOpenDocuments = () => {
        ...
        if (
            active &&
            open.some(
                (document) => document.uri.fsPath === activePath
            )
        ) {
